...
export class ImageEncoder {
    ...
    private sampleProjectionFeatures(
        ...
        for (let i = 0; i < vertexCount; i++) {
            ...
            // Clamp to bounds (padding_mode='border')
            const u_clamped = Math.max(0, Math.min(mapSize - 1 - 1e-6, u));
            const v_clamped = Math.max(0, Math.min(mapSize - 1 - 1e-6, v));
            
            if (u >= 0 && u <= mapSize - 1 && v >= 0 && v <= mapSize - 1) {
                validCount++;
            }

            const x0 = Math.floor(u_clamped);
            const y0 = Math.floor(v_clamped);
            const x1 = Math.min(x0 + 1, mapSize - 1);
            const y1 = Math.min(y0 + 1, mapSize - 1);

            const wx = u_clamped - x0;
            const wy = v_clamped - y0;
