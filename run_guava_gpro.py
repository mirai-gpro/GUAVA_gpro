...
image = (
    ...
    .apt_install(
        # modal_final_clean.py pattern + CUDA build requirements
        "git", "libgl1-mesa-glx", "libglib2.0-0", "ffmpeg", "wget",
        "libsm6", "libxext6", "libxrender-dev", "libusb-1.0-0", "libasound2",
    ...
    # 3. Install chumpy first (needs --no-build-isolation)
    .run_commands("pip install chumpy==0.70 --no-build-isolation")

    # 4. Install requirements.txt (as README says)
    .add_local_file("./requirements.txt", remote_path="/tmp/requirements.txt", copy=True)
    .run_commands("pip install -r /tmp/requirements.txt --no-build-isolation")

    # 5. Install PyTorch3D (as README says)
    .run_commands("pip install git+https://github.com/facebookresearch/pytorch3d.git@v0.7.7 --no-build-isolation")

    # 6. Submodules Build (as README says: cd submodules && pip install ...)
...
def run_guava():
    ...
    # modal_final_clean.py pattern: stream output
    env = os.environ.copy()
